...
! defined( 'WOOPT_VERSION' ) && define( 'WOOPT_VERSION', '5.1.1' );
		...
		if ( ! class_exists( 'WPCleverWoopt' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWoopt {
				...
				function __construct() {
					...
					add_action( 'wp_ajax_woopt_add_time', [ $this, 'add_time' ] );
					add_action( 'wp_ajax_woopt_add_apply_combination', [ $this, 'add_apply_combination' ] );
								...
								foreach ( $apply_val['combined'] as $conditional_item ) {
									...
									if ( $conditional_item_type === 'variation' ) {
										...
										if ( $is_variation ) {
											...
											if ( has_term( $conditional_item_val, $conditional_item_type, $product_id ) ) {
												$match = true;
											}
										}
									}
				...
				function save_actions() {
					check_ajax_referer( 'woopt-security', 'nonce' );
				...
				function add_apply_combination() {
					check_ajax_referer( 'woopt-security', 'nonce' );
